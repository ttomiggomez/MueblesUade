...
class ListarProductos extends React.Component{
    ...
    renderList(){
          let preciosBajos = this.state.productos.filter(productos => productos.precio < 65000);
        return preciosBajos.map((elem) => {
            return(
                  < Producto id = {elem.id} descripcion = {elem.descripcion} nombre = {elem.nombre} imagenes ={elem.imagenes} precio = {elem.precio} rating = {elem.rating} />
                                );
        });
    }
    render(){
